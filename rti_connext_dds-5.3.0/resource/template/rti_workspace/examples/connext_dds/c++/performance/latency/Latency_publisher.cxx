...
int RTI_STRCAT(char * command_line, const char * message) {
#ifdef RTI_WIN32
    int err = strcat_s(command_line, COMMAND_LINE_SIZE, message);
    return err;
#else
    char * err = strncat(command_line, message, COMMAND_LINE_SIZE);
    if (err == NULL) {
        return 1;
    } else {
        return 0;
...
int RTI_STRCPY(char * command_line, const char * message) {
#ifdef RTI_WIN32
    int err = strcpy_s(command_line, COMMAND_LINE_SIZE, message);
    return err;
#else
    char * err = strncpy(command_line, message, COMMAND_LINE_SIZE);
    if (err == NULL) {
        return 1;
    }
    else {
...
int publisher_vx_main(int argc, char **argv)
#endif
{
    ...
    if(argc <= 1){
      ...
      int err;
      err = RTI_STRCPY(command_line, argv[0]);
      if (err != 0) {
          AppLog_exception("There is not enough space in the command_line "
              "variable to copy the content of the command line arguments.\n");
          return 0;
      }
      for (i = 1; i < argc; i++) {
          err = RTI_STRCAT(command_line, " ");
          if (err != 0) {
              AppLog_exception("There is not enough space in the command_line "
                  "variable to copy the append of the command line arguments.\n");
              return 0;
          }
          err = RTI_STRCAT(command_line, argv[i]);
          if (err != 0) {
              AppLog_exception("There is not enough space in the command_line "
                  "variable to copy the append of the command line arguments.\n");
              return 0;
