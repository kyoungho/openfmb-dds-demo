...
int RTI_LOCALTIME(struct tm** lt) {
	time_t now_time = 0;
#ifdef RTI_WIN32
	int err;
#endif
	...
	*lt = localtime(&now_time);
	if (*lt == NULL) {
		printf("There was an error in localtime.");
		return 1;
	}
#endif
	return 0;
...
DDS_Boolean read_articles(
...
#ifdef RTI_WIN32
    struct tm* lt = (struct tm*) calloc(sizeof(struct tm), 1);
    if (lt == NULL) {
    ...
    for (elapsedMillis = 0;
        ...
        err = RTI_LOCALTIME(&lt);
        if (err == 0) {
            printf(
                    "Available articles as of %02d:%02d:%02d:\n",
                    lt->tm_hour,
                    lt->tm_min,
                    lt->tm_sec);
        }
