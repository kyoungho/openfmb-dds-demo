...
HelloWorldPlugin::print_data(
    ...
    DataPrintUtility::print_object(
        sample->prefix, "prefix", indent_level);

    DataPrintUtility::print_object(
        sample->sampleId, "sampleId", indent_level);

    DataPrintUtility::print_objects(
        sample->payload, "payload", indent_level);
    ...
    Object^ endpoint_plugin_qos)
{
    ...
    if (serialize_sample) {
        ...
        if (!stream.serialize_string(sample->prefix  , ((HELLODDS_MAX_STRING_SIZE::VALUE)))) {
            return false;
        }
        if (!stream.serialize_long(sample->sampleId )) {
            return false;
        }
        if (!stream.serialize_primitive_sequence(sample->payload)) {
            return false;
        }
    ...
    Object^ endpoint_plugin_qos)
{
    ...
    if (deserialize_data) {
        ...
        try{
            ...
            sample->prefix = stream.deserialize_string( ((HELLODDS_MAX_STRING_SIZE::VALUE)));
            if (sample->prefix  == nullptr) {
                return false;
            }
            sample->sampleId = stream.deserialize_long();
    ...
    Object^ endpoint_plugin_qos)
{
    ...
    if (skip_sample) {
        if (!stream.skip_string(((HELLODDS_MAX_STRING_SIZE::VALUE)))) {
            return false;
        }
        if (!stream.skip_long()) {
            return false;
        }
        if (!stream.skip_primitive_sequence(
            RTI_CDR_OCTET_TYPE)) {
            return false;
        }
    }
    ...
    UInt32 current_alignment)
{
    ...
    current_alignment +=CdrSizes::STRING->serialized_size(
        current_alignment , ((HELLODDS_MAX_STRING_SIZE::VALUE))+1);

    current_alignment +=CdrSizes::LONG->serialized_size(
        current_alignment );

    current_alignment +=  CdrSizes::OCTET->serialized_sequence_size(
        current_alignment, ((HELLODDS_MAX_PAYLOAD_SIZE::VALUE)) );
    ...
    HelloWorld^ sample)
{
    ...
    current_alignment += CdrSizes::STRING->serialized_size(
        endpoint_data->get_alignment(current_alignment), sample->prefix);
