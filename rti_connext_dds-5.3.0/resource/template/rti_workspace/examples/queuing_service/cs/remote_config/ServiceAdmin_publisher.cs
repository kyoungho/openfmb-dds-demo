    ...
    */
    public static String sampleIdentityToString(RTI.QueuingService.SampleIdentity_t sampleIdentity)
    {
        ...
        for (int i = 0; i <= 15; i++)
        {
            stringBuilder.AppendFormat(
                "{0,0:X2}",
                sampleIdentity.writer_guid._cs_value.GetValue(i));
        }
    ...
    {
        Console.WriteLine("Usage: -d <domainId> -q <serviceName> -o <profileName> [create|delete|get|shutdown] <resourceIdentifier> {command options}");
        ...
        for (int i = 0; i < args.Length; i++)
        {
            ...
            else if (args[i].Equals("-r"))
            {
                ...
                if (!isActionFound)
                {
                    ...
                    else if (requestAction.Equals("shutdown"))
                    {
                        action = CommandActionKind.RTI_SERVICE_COMMAND_ACTION_SHUTDOWN;
                        resourceIdentifier = "/";
                    }
                    else
                    {
