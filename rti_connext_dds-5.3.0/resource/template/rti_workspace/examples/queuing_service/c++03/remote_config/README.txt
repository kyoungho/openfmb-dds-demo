...
2. This example requires you to generate code from two different IDL files:
   ...
   * QueuingServiceTypes.idl contains the types required to parse
   ...
   choice (e.g., x64Linux2.6gcc4.4.5) on ServiceAdmin.idl. 
   ...
   $(NDDSHOME)/bin/rtiddsgen.bat -example <arch> -language C++03 -namespace -unboundedSupport -ppDisable ServiceAdmin.idl
   ...
   After running rtiddsgen, you will see messages that look like this:
	...
	CPP2\remoteConfig\ServiceAdmin_publisher.cxx
...
3. Run rtiddsgen on QueuingServiceTypes.idl:
   ...
   $(NDDSHOME)/bin/rtiddsgen.bat -language C++03 -namespace -unboundedSupport -ppDisable QueuingServiceTypes.idl

4. Open the makefile/solution generated at step 2 and add the files generated in the 
   previous.

   4.a Unix/Makefile:
   	- Open the generated makefile and add QueuingServiceTypesImplPlugin.cxx QueuingServiceTypesImpl.cxx QueuingServiceTypes.cxx to the
	COMMONSOURCES variable

	- Add "-lrticonnextmsgcpp2z" to the LIBS variable just before the "-lnddscpp2z" library
   ...
   4.b Visual studio:
	...
	- Since RTI Connext Messaging is required, you need to add the the library rticonnextmsgcpp2[z][d].lib to the ServiceAdmin_publisher
...
2. Create a SharedReaderQueue in an existing SharedSubscriber. You can use the 
   topic and type names used by your producer and consumer application:
     ...
     ServiceAdmin_publisher.exe  \
	  ...
	  -p HelloWorldTopic -t HelloWorldRequest 
